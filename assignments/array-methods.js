...
let ticketPriceTotal = donations.reduce( function(x, y) {
    return sum(x,y)
});
...
// Problem 1
// information of people who donated more than 100.
let largeDonations = runners.filter(runner => runner.donation > 100);
console.log(largeDonations);

// Problem 2
// new array of companies represented. Sorted Alpabetically.
let companiesRepresented = [];
runners.forEach(runner=> {
   if ( !companiesRepresented.includes(runner.company_name) ) {
       companiesRepresented.push(runner.company_name);
   }
})
console.log(companiesRepresented.sort());
